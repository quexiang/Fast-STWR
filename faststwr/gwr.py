# Main GWR,MGWR and STWR classes
...
class STWR(GLM):
    ...
    def __init__(self, coords_list,y_list, X_list,
                 ...
                 fixed=False, constant=True,spherical=False,compress = True,max_cal_tol =100000000,recorded = 0):
        ...
        X_arr = self.X_list[-1]
        self.cur_len = len(coords_list[-1])
        self.cal_len_tol = len(coords_list[-1])
        for i in range(self.tick_nums-1):
             ...
             self.cal_len_tol += len(self.coords_list[-(2+i)])
           
        for x_iter in range(X_arr.shape[1]):
            jd_x_tick = X_arr[:,x_iter:(x_iter+1)].copy()
            jd_x_tick = (jd_x_tick != 0).astype(np.float32)
            jd_x_tick = np.sum(jd_x_tick)
            if(jd_x_tick< X_arr.shape[1]):
                 raise NameError("init X Error")
        try:              
            GLM.__init__(self, y_arr, X_arr, family, constant=constant)
        except:
            raise NameError("init GLM Error")
        else:
            self.y_arr = y_arr
            self.X_arr = X_arr
            self.constant = constant
            ...
            # If cur_len by cal_len_tol is greater than max_cal_tol(default is 10000*10000) 
            # then the storage memory will be separated into small parts.
            if(self.cur_len*self.cal_len_tol > max_cal_tol ):
                 max_tick = math.ceil(max_cal_tol*1.0/self.cal_len_tol)
                 self.separts = math.ceil(self.cur_len*1.0/max_tick)
                 self.last_coords =  self.coords_list[-1].copy()
                 self.last_coords = np.array_split(self.last_coords, self.separts, axis = 0)
            ...
            self.aspList = None
            self.atpList = None
            self.PretmpList = None
            self.WPrelist = None
            if(self.separts is None):
                self.W = self._build_spt_W(fixed, kernel,coords_list,self.y_list,tick_times_intervel,sita,tick_nums,gwr_bw0=self.gwr_bw0,points_list=self.points_list,alpha = self.alpha)    
            else:
                self.Wlist = []
                cur_recorded = self.recorded
                if(cur_recorded == 0):# no recorded for pred
                    self.recorded = 3 # sperated recorded for search
                    self.sep_ystar_List =[0]
                    for i in range(self.separts):
                        wi_tick = self._build_spt_W(self.fixed, self.kernel,self.coords_list,
                                                             self.y_list,self.tick_times_intervel,self.sita,self.tick_nums,gwr_bw0=self.gwr_bw0,
                                                             last_coords = [self.last_coords[i]],
                                                             points_list = self.points_list,
                                                             alpha = self.alpha,build_asp = True,sep_y_star=self.sep_ystar_List[i])
                        self.Wlist.append(wi_tick)
                        self.sep_ystar_List.append(wi_tick.shape[0])
                    self.recorded = cur_recorded
                elif(cur_recorded == 5):#sperate_pred no flash fitted dtmplist 
#                     self.recorded = 5
                     self.sep_Prestar_List =[0]
                     self.WPrelist =[]
                     for i in range(self.separts):
                         wi_tick = self._build_spt_W(self.fixed, self.kernel,self.coords_list,
                                                             self.y_list,self.tick_times_intervel,self.sita,self.tick_nums,gwr_bw0=self.gwr_bw0,
                                                             last_coords = [self.last_coords[i]],
                                                             points_list = self.points_list,
                                                             alpha = self.alpha,build_asp = True,atpList = self.atpList,sep_y_star=self.sep_Prestar_List[i],pred = self.pred)
                         self.WPrelist.append(wi_tick)
                         self.sep_Prestar_List.append(wi_tick.shape[0])
                else:# recorded for pred (recorded ==1)
                    self.recorded = 4
                    self.sep_ystar_List =[0]
                    for i in range(self.separts):
                        wi_tick = self._build_spt_W(self.fixed, self.kernel,self.coords_list,
    ...
    def _build_spt_W(self, fixed, kernel,coords_list,y_list,tick_times_intervel,sita,tick_nums, gwr_bw0=None,last_coords = None,
                     points_list=None,alpha=0.3,build_asp=False,aspList = None,atpList = None,sep_y_star=0,pred = False):

        if self.recorded == 1:#no_spearted_pred
             W,self.dtmat,self.d_tmp_list = ak[kernel](coords_list,y_list,tick_times_intervel,self.X_list,sita,tick_nums,
                                           gwr_bw0,last_coords,self.dspal_mat_list,
                                           self.sorted_dspal_list,
                                           self.d_tmp_list,
                                           self.dspmat,self.dtmat,points_list =points_list ,alpha=alpha,
                                           spherical=self.spherical,build_asp = build_asp,aspList = aspList,atpList=atpList,sep_y_star=sep_y_star,pred = pred,rcdtype=self.recorded)
             return W
        elif self.recorded == 2:#save space spearted_search
              if(self.aspList  is None ):
                   self.aspList = []
              W,asp_list = ak[kernel](coords_list,y_list,tick_times_intervel,self.X_list,sita,tick_nums,
                                           gwr_bw0,last_coords,self.dspal_mat_list,
                                           self.sorted_dspal_list,
                                           self.d_tmp_list,
                                           self.dspmat,self.dtmat,points_list =points_list ,alpha=alpha,
                                           spherical=self.spherical,build_asp = build_asp,aspList = aspList,atpList=atpList,sep_y_star=sep_y_star,pred = pred,rcdtype=self.recorded)
              self.aspList.append(asp_list)
              return W
        elif self.recorded == 3:#spearted_search
              if(self.aspList  is None )&(self.atpList is None):
                   self.aspList = []
                   self.atpList = []   
              W,asp_list,atp_list = ak[kernel](coords_list,y_list,tick_times_intervel,self.X_list,sita,tick_nums,
                                           gwr_bw0,last_coords,self.dspal_mat_list,
                                           self.sorted_dspal_list,
                                           self.d_tmp_list,
                                           self.dspmat,self.dtmat,points_list =points_list ,alpha=alpha,
                                           spherical=self.spherical,build_asp = build_asp,aspList = aspList,atpList=atpList,
                                           sep_y_star=sep_y_star,pred = pred,rcdtype=self.recorded)
              self.aspList.append(asp_list)
              self.atpList.append(atp_list)
              return W
        elif self.recorded == 4: #spearted_pred
              if(self.atpList is None):
                   self.atpList = []   
              W,atp_list = ak[kernel](coords_list,y_list,tick_times_intervel,self.X_list,sita,tick_nums,
                                           gwr_bw0,last_coords,self.dspal_mat_list,
                                           self.sorted_dspal_list,
                                           self.d_tmp_list,
                                           self.dspmat,self.dtmat,points_list =points_list ,alpha=alpha,
              ...
              self.atpList.append(atp_list)
              return W
        elif  self.recorded == 5:
#                 if(self.PretmpList is None):
#                        self.PretmpList = []   
                 W = ak[kernel](coords_list,y_list,tick_times_intervel,self.X_list,sita,tick_nums,
                                               gwr_bw0,last_coords,self.dspal_mat_list,
                                               self.sorted_dspal_list,
                                               self.d_tmp_list,
                                               self.dspmat,self.dtmat,points_list =points_list ,alpha=alpha,
...
#                 self.PretmpList.append(Pretmp_list)
                 return W
        else:
                if self.compress:
                    if fixed:
                        try:
                            W = fk[kernel](coords_list,y_list,tick_times_intervel,self.X_list,sita,tick_nums,
                                           gwr_bw0,last_coords,self.dspal_mat_list,
                                           self.sorted_dspal_list,
                                           self.d_tmp_list,
                                           self.dspmat,self.dtmat,points_list =points_list ,alpha=alpha,
                                           spherical=self.spherical,build_asp = build_asp,aspList=aspList,atpList=atpList,
                                           sep_y_star=sep_y_star,pred = pred)
                        except BaseException:
                            raise  # TypeError('Unsupported kernel function  ', kernel)
                    else:
                        try:
    ...
    def fit(self, ini_params=None, tol=1.0e-5, max_iter=20,
        ...
        if solve.lower() == 'iwls':
                    ...
                    for i in range(m):
                        ...
                        if isinstance(self.family, Gaussian): 
                            betas, inv_xtx_xt = _compute_betas_gwr(
                                     self.y, self.X, wi)
                            influ[i] = np.dot(self.X[i], inv_xtx_xt[:, i])
                            predy[i] = np.dot(self.X[i], betas)[0]
                            resid[i] = self.y[i] - predy[i]
                        elif isinstance(self.family, (Poisson, Binomial)):
                            rslt = iwls(self.y, self.X, self.family,
                                        self.offset, None, ini_params, tol,
                                        max_iter, wi=wi)
                            ...
                            predy[i] = rslt[1][i]
                            resid[i] = self.y[i] - predy[i]
                    return STWRResultsLite(self, resid, influ,m,predy)#,S,w)
                else:
                    params = np.zeros((m, self.k))
                    predy = np.zeros((m, 1))
                    w = np.zeros((m, 1))
            ...
            else:#self.separts is not None
                if searching:  
                    if  self.threadnums is not None:
                        resid_tol = {}
                        influ_tol = {}
                        predy_tol = {}

                        separr_tred = np.arange(self.separts)
                        if(self.separts<self.threadnums):
                            self.threadnums = self.separts
                        separr_tred = np.array_split(separr_tred, self.threadnums)
                        m_tol_dc ={}
                        for spt_i in sorted (self.Wdict): 
                            m_tol_dc[spt_i] = self.Wdict[spt_i].shape[0]
                        
                        ths = []
                        for i in range(self.threadnums):
                             th = threading.Thread(target =self.thread_cal_results, args = (separr_tred[i],m_tol_dc,resid_tol,influ_tol,predy_tol))
                             th.start()
                             ths.append(th)
                        for th in ths:
                                th.join()                          
                        m_tol = sum(m_tol_dc.values())
                        cal_resid_tol = []
                        cal_influ_tol = []
                        cal_predy_tol = []
                        for vlist in range(self.separts):
                             cal_resid_tol.extend(resid_tol[vlist])
                             cal_influ_tol.extend(influ_tol[vlist])
                             cal_predy_tol.extend(predy_tol[vlist])
                        
                        resid_tol = np.asarray(cal_resid_tol).reshape((m_tol, 1))
                        influ_tol = np.asarray(cal_influ_tol).reshape((m_tol, 1))
                        predy_tol = np.asarray(cal_predy_tol).reshape((m_tol, 1))
                        return STWRResultsLite(self, resid_tol, influ_tol,m_tol,predy_tol)#,S,w)
                    else:
                        m_tol = 0
                        resid_tol = []
                        influ_tol = []
                        predy_tol = []
                        for spt_iter in range(self.separts):
                            m = self.Wlist[spt_iter].shape[0]
                            resid = np.zeros((m, 1))
                            influ = np.zeros((m, 1))
                            predy = np.zeros((m, 1))                    
                            for i in range(m):
                                wi = self.Wlist[spt_iter][i].reshape((-1, 1))
                                if isinstance(self.family, Gaussian): 
                                    betas, inv_xtx_xt = _compute_betas_gwr(
                                             self.y, self.X, wi)
                                    influ[i] = np.dot(self.X[i+m_tol], inv_xtx_xt[:,i+m_tol])
                                    predy[i] = np.dot(self.X[i+m_tol], betas)[0]
                                    resid[i] = self.y[i+m_tol] - predy[i]
                                elif isinstance(self.family, (Poisson, Binomial)):
                                    rslt = iwls(self.y, self.X, self.family,
                                                self.offset, None, ini_params, tol,
                                                max_iter, wi=wi)
                                    ...
                                    influ[i] = np.dot(self.X[i+m_tol], inv_xtx_xt[:,i+m_tol]) * \
                                               rslt[3][i+m_tol][0]
                                    predy[i] = rslt[1][i+m_tol]
                                    resid[i] = self.y[i+m_tol] - predy[i]
                            resid_tol.extend(resid.tolist())
                            influ_tol.extend(influ.tolist())
                            predy_tol.extend(predy.tolist())
                            m_tol += m
                        resid_tol = np.asarray(resid_tol).reshape((m_tol, 1))
                        influ_tol = np.asarray(influ_tol).reshape((m_tol, 1))
                        predy_tol = np.asarray(predy_tol).reshape((m_tol, 1))
                        return STWRResultsLite(self, resid_tol, influ_tol,m_tol,predy_tol)
                elif(self.WPrelist is not None):
                            m = self.WPrelist[-1].shape[0]
                            params = np.zeros((m, self.k))
                            predy = np.zeros((m, 1))
                            w = np.zeros((m, 1))
                            S = np.zeros((m, self.n))
                            CCT = np.zeros((m, self.k))
                            for i in range(m):
                                wi = self.WPrelist[-1][i].reshape((-1, 1))
                                rslt = iwls(self.y, self.X, self.family,
                                            self.offset, None, ini_params, tol,
                                            max_iter, wi=wi)
                                params[i, :] = rslt[0].T
                    ...
                    for spt_iter in range(self.separts):
                        ...
                        CCT = np.zeros((m, self.k))
                        for i in range(m):
                            wi = self.Wlist[spt_iter][i].reshape((-1, 1))#[:m,]
                            rslt = iwls(self.y, self.X, self.family,
                                        self.offset, None, ini_params, tol,
                                        max_iter, wi=wi)
                            params[i, :] = rslt[0].T
                            predy[i] = rslt[1][i+m_tol]
                            w[i] = rslt[3][i+m_tol]
                            S[i] = np.dot(self.X[i+m_tol], rslt[5])
                            CCT[i] = np.diag(np.dot(rslt[5], rslt[5].T))
                        if binit:
                            params_tol = np.vstack((params_tol,params))
                            S_tol = np.vstack((S_tol,S))
                            CCT_tol = np.vstack((CCT_tol,CCT))
                        else:
                            params_tol = params
                            S_tol = S
                            CCT_tol = CCT
                            binit = True
                        predy_tol.extend(predy.tolist())
                        w_tol.extend(w.tolist())
                        m_tol += m                    
                    predy_tol = np.asarray(predy_tol).reshape((-1, 1))
                    w_tol =np.asarray(w_tol).reshape((-1, 1))
                    return STWRResults(self, params_tol, predy_tol, S_tol, CCT_tol,w_tol,compress = self.compress)

    def thread_cal_results(self,separr,m_tol_dc,resid_tol,influ_tol,predy_tol):
                    m_t0 = separr[0] 
                    m_tol = 0
                    for k in m_tol_dc.keys():
                        if k < m_t0 :
                            m_tol += m_tol_dc[k]
                    lenseppar = len(separr)
                    for ss in range(lenseppar):
                        idx = separr[ss]

                        m = self.Wdict[idx].shape[0]
                        resid = np.zeros((m, 1))
                        influ = np.zeros((m, 1))
                        predy = np.zeros((m, 1))                    
                        for i in range(m):
                            wi = self.Wdict[idx][i].reshape((-1, 1))
                            if isinstance(self.family, Gaussian): 
                                betas, inv_xtx_xt = _compute_betas_gwr(
                                         self.y, self.X, wi)
                                influ[i] = np.dot(self.X[i+m_tol], inv_xtx_xt[:,i+m_tol])
                                ...
                                rslt = iwls(self.y, self.X, self.family,
                                            self.offset, None, self.ini_params, self.tol,
                                            self.max_iter, wi=wi)
                                inv_xtx_xt = rslt[5]
                                influ[i] = np.dot(self.X[i+m_tol], inv_xtx_xt[:,i+m_tol]) * \
                                           rslt[3][i+m_tol][0]
                                predy[i] = rslt[1][i+m_tol]
                                resid[i] = self.y[i+m_tol] - predy[i]
                        resid_tol[idx] = resid.tolist()
                        influ_tol[idx] = influ.tolist()
                        predy_tol[idx] = predy.tolist()
                        m_tol += m
        
    def searchfit_spt(self,_opt_bsita_tick,gwr_bw0_tick,alpha,searching = True,fit_params={}):
        self.alpha = alpha
        self.sita = _opt_bsita_tick
        self.gwr_bw0 = gwr_bw0_tick
        if(self.separts is None):
            self.W = self._build_spt_W(self.fixed, self.kernel,self.coords_list,self.y_list,self.tick_times_intervel,self.sita,self.tick_nums,
                                        gwr_bw0 =self.gwr_bw0,points_list = self.points_list,alpha = self.alpha)    
        else:
            self.dspal_mat_list = None
            self.sorted_dspal_list = None
            self.d_tmp_list  =None 
            self.dspmat= None
            self.dtmat =None
            self.Wlist = []
            if  self.threadnums is not None:
                ...
                ths = []
                for i in range(self.threadnums):
                     th = threading.Thread(target = self._build_Tred_spt_W, args = (separr_tred[i], ))
                     th.start()
                     ths.append(th)
                for th in ths:
                        th.join()
                ...
                for i in range(self.separts):
                    self.Wlist.append( self._build_spt_W(self.fixed, self.kernel,self.coords_list,
                                                         self.y_list,self.tick_times_intervel,self.sita,self.tick_nums,
                                                         gwr_bw0=self.gwr_bw0,last_coords = [self.last_coords[i]],
                                                         points_list = self.points_list,alpha = self.alpha,
                                                         build_asp = True,aspList =self.aspList[i],atpList = self.atpList[i]))

        
        stwr = self.fit(searching = searching,**fit_params)
    
        return stwr 

        
    def _build_Tred_spt_W(self,sep_arr):

        
        for ss in range(sep_arr.shape[0]):
            tick_i = sep_arr[ss]
            self.Wdict[tick_i] =  self._build_spt_W(self.fixed, self.kernel,self.coords_list,
                                                     self.y_list,self.tick_times_intervel,self.sita,self.tick_nums,
                                                     gwr_bw0=self.gwr_bw0,last_coords = [self.last_coords[tick_i]],
                                                     points_list = self.points_list,alpha = self.alpha,
                                                     build_asp = True,aspList =self.aspList[tick_i],atpList = self.atpList[tick_i])
    ...
    def predict(self, points_list, P,  exog_scale=None, exog_resid=None,
        ...
        if self.separts is None :
            ...
            l_pred = len(self.points_list[0])
            if(l_pred*self.cal_len_tol >self.max_cal_tol ):
                raise NotImplementedError(
                        'Not yet implemented for too many points predit')
            Pre_P = P[0]
            self.recorded = 5 #separted pred, no flash previous fitted dtmplist

            if self.constant:
                    Pre_P = np.hstack([np.ones((len(Pre_P), 1)), Pre_P])
                    self.P = Pre_P
            else:
                self.P = Pre_P
            
            if(self.WPrelist is None):
                self.WPrelist = []
            
            wi_tick = self._build_spt_W(self.fixed, self.kernel,self.coords_list,
                                                     self.y_list,tick_times_intervel=self.tick_times_intervel,
                                                     sita = self.sita,tick_nums = self.tick_nums,gwr_bw0=self.gwr_bw0,
                                                     last_coords = [self.last_coords[0]],
                                                      points_list = self.points_list,
                                                     alpha = self.alpha,build_asp = True,atpList = self.atpList,sep_y_star=0,pred = True)
            self.WPrelist.append(wi_tick)
...
class STWRResultsLite(object):
    def __init__(self, model, resid, influ,m,predy):
        self.m = m
        self.y = model.y[:m] 
        self.family = model.family
        self.n = m
...
class STWRResults(GLMResults):
    def __init__(self, model, params, predy, S, CCT,w=None,compress = False):        
        ...
        if model.separts is None:     
            ...
            if(compress):
                self.model_copy =copy.deepcopy(model)
                if(model.WPrelist is None):#pred
                    mwsize =0
                    initw0ed = False
                    for witer in range(self.model_copy.separts):
                         mwsize +=len(self.model_copy.Wlist[witer])
                         if(initw0ed):
                             self.model_copy.W = np.vstack((self.model_copy.W,self.model_copy.Wlist[witer]))
                         else:
                             self.model_copy.W =self.model_copy.Wlist[witer] 
                             initw0ed = True
                         
                    m = mwsize
                else:#pred
                    m = model.WPrelist[-1].shape[0]
                    self.model_copy.W  = self.model_copy.WPrelist[-1]
                self.y = self.model_copy.y[:m]
                self.n = m
                self.model_copy.y = self.model_copy.y[:m]
                self.model_copy.X = self.model_copy.X[:m] 
    ...
    def predictions(self):
        ...
        if P is None:
            raise TypeError('predictions only avaialble if predict'
                            'method is previously called on STWR model')
        else:
            predictions = np.sum(P * self.params, axis=1).reshape((-1, 1))
        return predictions
